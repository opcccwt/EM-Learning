...
class Network:
    ...
    def hugin_net(self):
        ...
        for var,size in enumerate(self.var_sizes):
            name = var_fmt % var
            states = " ".join( '"s%d"' % i for i in range(size))
            st += "node %s\n{\n    states = (%s);\n}\n" % (name,states)
        st += "\n"
    ...
    def mplp_format(self):
        ...
        for domain in self.pot_domains:
            ...
            regions += "\n"
        # all node domains
        for i in range(len(self.var_sizes)):
            regions += "%d\n" % (i+1)
...
def topological_order(bn):
    ...
    var2pot = [ set() for x in range(bn.size) ]
...
def is_topologically_ordered(bn):
    ...
    for i,pot_vars in enumerate(bn.pot_domains):
        for var in pot_vars:
            var2pot[var].add(i)
    for var in range(bn.size-1,0,-1):
...
def parse_uai(filename):
    ...
    size = int(t.next())
    var_sizes = [ int(t.next()) for var in range(size) ]
    num_pots = int(t.next())
    pot_domains = []
    for i in range(num_pots):
        pot_size = int(t.next())
        domain = [ int(t.next()) for var in range(pot_size) ]
        pot_domains.append(domain)

    pots = []
    for i in range(num_pots):
        pot_length = int(t.next())
        pot = [ t.next() for val in range(pot_length) ]
        pots.append(pot)
    return Network(network_type,var_sizes,pot_domains,pots)
...
def tuples(my_list,k):
    ...
    it = iter(my_list)
    for x in it:
        cur = [x] + [ it.next() for x in range(k-1) ]
        yield tuple(cur)
